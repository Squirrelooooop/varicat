...
webshot("varicat_ontology.html", "r-small-1.png") %>%
resize("75%")
}
...
for (i in l2_exist_in_inflam) { 
 a <- inflam %>% group_by(.data[[i]],  ) %>% filter(!is.na(.data[[i]])) %>% purrr::discard(~all(is.na(.)))
#a <- cbind(a[1], mycol = do.call(pmax, c(a[-1], na.rm = TRUE)))
 print(a)
}
